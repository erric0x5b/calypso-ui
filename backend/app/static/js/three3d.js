...
export function init3DOnce() {
  ...
  const ro = new ResizeObserver(() => {
    const w = host.clientWidth || 0; const h = host.clientHeight || 0;
    if (w < 10 || h < 10)
      return;
    camera.aspect = w / h;
  ...
  loader.load("models/rov.glb", (gltf) => {
    ...
    const s = 1.0 / maxDim; model.scale.setScalar(s);
    scene.add(model); rov3d.model = model;
    const fov = camera.fov * (Math.PI / 180);
    let camZ = Math.abs(1 / (2 * Math.tan(fov / 2)));
    camZ *= 1.8; camera.position.set(0, 0.4, camZ); camera.lookAt(0, 0, 0);
    camera.updateProjectionMatrix();
  }, undefined, (err) => console.error('[3D] load error', err));
}

export function setRovAttitudeRad(roll, pitch, yaw) {
  if (!rov3d.model)
    return;
